...
export class RedisMock {
  ...
  async ltrim(key: string, start: number, stop: number): Promise<'OK'> {
    ...
    const arr: string[] = JSON.parse(e.value)
    const end = stop === -1 ? arr.length : stop + 1
    store.set(key, { value: JSON.stringify(arr.slice(start, end)) })
    return 'OK'
  }
  ...
  async llen(key: string): Promise<number> {
    const e = live(key)
    if (!e) return 0
    return (JSON.parse(e.value) as string[]).length
  }
  ...
  async geodist(
    ...
    const entries = geoStore.get(key) ?? []
    const e1 = entries.find((x) => x.member === member1)
    const e2 = entries.find((x) => x.member === member2)
    if (!e1 || !e2) return null
    const km = haversineKm(e1.lat, e1.lng, e2.lat, e2.lng)
    const dist =
      unit === 'km' ? km
      : unit === 'm'  ? km * 1000
      : unit === 'mi' ? km / 1.60934
      : km * 3280.84
    return String(Math.round(dist * 100) / 100)
  }
